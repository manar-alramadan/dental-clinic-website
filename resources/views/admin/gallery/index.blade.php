...
<form action="{{ route('admin.logout') }}" method="POST">
    ...
    <button
        type="submit"
        class="rounded-xl bg-red-400/50 px-5 py-3 font-bold text-red-200 transition hover:bg-red-500 hover:text-white"
    >
        تسجيل الخروج
    </button>
</form>
                ...
                @foreach($images as $image)
                    ...
                    <article class="overflow-hidden rounded-3xl border border-white/10 bg-white/5 shadow-xl">
                        ...
                        <div class="aspect-square overflow-hidden bg-slate-900">
                            ...
                            <img
                                src="{{ str_starts_with($image->image, 'images/') ? asset($image->image) : asset('storage/' . $image->image) }}"
